...
They are defined for current type.
They are defined for the root type in the current file.
...
When the button is clicked, the button emits the Button.clicked() signal, which we connected to updateText. The signal then invokes updateText, which updates the counter and the text on the label.
...
In this example, we listen for changes to the CheckBox.checkState property of the CheckBox using its change signal, checkStateChanged with the signal handler onCheckStateChanged.
...
In this example, the button’s Button.text property is re-evaluated every time the button is clicked, because the clicks property has changed.
