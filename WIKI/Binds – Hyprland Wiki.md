...
hl.bind("SUPER \+ SHIFT \+ Q", hl.dsp.exec\_cmd("firefox")) 
...
hl.bind("Print", hl.dsp.exec\_cmd("grim")) 
...
See the xkbcommon-keysyms.h header for all the keysyms. The name you should use is the segment after XKB\_KEY\_ . 

If you want to bind by a keycode, you can put it in the KEY position with a code: prefix, e.g.: hl.bind("SUPER \+ code:28", hl.dsp.exec\_cmd("amongus"))

This will bind SUPER \+ t since t is keycode 28\. 

Note 

If you are unsure of what your key’s name or keycode is, you can use wev to find out. Bind flags   
...
hl.bind(keys, hl.dsp.exec\_cmd("amongus"), { release **\= true**, locked **\= true** }) 
...
| click | Will trigger on release of a key or button as long as the mouse cursor stays inside binds:drag\_threshold . |
| drag | Will trigger on release of a key or button as long as the mouse cursor moves outside binds:drag\_threshold . |
| long\_press  | Will trigger on long press of a key. |
| repeating  | Will repeat when held. |
| non\_consuming  | Key/mouse events will be passed to the active window in addition to triggering the dispatcher. |
| auto\_consuming  | Key/mouse events will be passed to the active window if the dispatcher doesn’t succeed. |
| mouse  | See the dedicated Mouse Binds section. |
| transparent  | Cannot be shadowed by other binds. |
| ignore\_mods  | Will ignore modifiers. |
| description  | Will allow you to write a description for your bind. |
| bypass  | Bypasses the app’s requests to inhibit keybinds. |
| submap\_universal  | Will be active no matter the submap. |
| device  | Allow binds to be set per device. See Per-Device Binds |

Example Usage: 

\-- Example volume button that allows press and hold, volume limited to 150% 

hl.bind("XF86AudioRaiseVolume", hl.dsp.exec\_cmd("wpctl set-volume \-l 1.5 @DEFAULT\_AUDIO\_SINK@ 5%+"), { re 

\-- Example volume button that will activate even while an input inhibitor is active hl.bind("XF86AudioLowerVolume", hl.dsp.exec\_cmd("wpctl set-volume @DEFAULT\_AUDIO\_SINK@ 5%-"), { locked **\=** 

\-- Open wofi on first press, closes it on second 

hl.bind("SUPER \+ SUPER\_L", hl.dsp.exec\_cmd("pkill wofi || wofi"), { release **\= true** }) 

\-- Skip player on long press and only skip 5s on normal press 

hl.bind("SUPER \+ XF86AudioNext", hl.dsp.exec\_cmd("playerctl next"), { long\_press **\= true** }) hl.bind("SUPER \+ XF86AudioNext", hl.dsp.exec\_cmd("playerctl position \+5")) 

Mouse buttons 

You can also bind or unbind mouse buttons by prefacing the mouse keycode with mouse: , e.g.: hl.bind("SUPER \+ mouse:272", hl.dsp.exec\_cmd("amongus")) \-- bind \`exec amogus\` to SUPER \+ LMB.
...
hl.bind("ALT \+ ALT\_L", hl.dsp.exec\_cmd("amongus"), { release **\= true** }) 

Mouse wheel 

You can also bind mouse wheel events with mouse\_up and mouse\_down (or mouse\_left and mouse\_right if your mouse supports horizontal scrolling): 

hl.bind("SUPER \+ mouse\_down", hl.dsp.focus({ workspace **\=** "e-1" })) 

Note 

You can control the reset time with hl.config.binds.scroll\_event\_delay . 
...
hl.bind("switch:\[switch name\]", hl.dsp.exec\_cmd("swaylock"), { locked **\= true** }) 

\-- Trigger when the switch is turning on. 

hl.bind("switch:on:\[switch name\]", hl.dsp.exec\_cmd("notify-send 'yooo'"), { locked **\= true** }) \-- Trigger when the switch is turning off. 

hl.bind("switch:off:\[switch name\]", hl.dsp.exec\_cmd("notify-send 'among us'"), { locked **\= true** }) 
...
hl.dispatch(hl.dsp.window.cycle\_next()) \-- Change focus to another window 

hl.dispatch(hl.dsp.window.bring\_to\_top()) \-- Bring it to the top 
...
hl.bind("SUPER \+ Q", hl.dsp.exec\_cmd("kitty"), { description **\=** "Open my favourite terminal" }) 
...
A list of devices is specified in the list field as a comma separated list of strings. Device tags may also be used in place of device names. See Devices. 

hl.bind(keys, dispatcher(params), { device **\=** { inclusive **\= true**, list **\=** { "device1", "device2" } } }) 

\-- Only example-keyboard-1 can use this bind 

hl.bind("SUPER \+ Q", hl.dsp.exec\_cmd("kitty"), { device **\=** { inclusive **\= true**, list **\=** { "example-keyboard- 

\-- Every keyboard other than razer-keyboard and asus-keyboard can use this bind 

hl.bind("SUPER \+ Q", hl.dsp.exec\_cmd("kitty"), { device **\=** { inclusive **\= false**, list **\=** { "razer-keyboard", 
...
hl.bind("ALT \+ mouse:272", hl.dsp.window.drag(), { mouse **\= true** }) \-- ALT \+ LMB: Move a window hl.bind("ALT \+ mouse:273", hl.dsp.window.resize(), { mouse **\= true** }) \-- ALT \+ RMB: Resize a window 
...
binds.drag\_threshold can be used to differentiate between clicks and drags with the same button: 

hl.config({ 

binds **\=** { 

drag\_threshold **\=** 10 \-- Fire a drag event only after dragging for more than 10px 

} 

}) 

hl.bind("ALT \+ mouse:272", hl.dsp.window.drag(), { mouse **\= true**, drag **\= true** }) \-- ALT \+ LMB (drag): M hl.bind("ALT \+ mouse:272", hl.dsp.window.float(), { mouse **\= true**, click **\= true** }) \-- ALT \+ LMB (click): 
...
hl.bind("SUPER \+ CTRL\_L", hl.dsp.window.drag(), { mouse **\= true** }) 

hl.bind("SUPER \+ mouse:273", hl.dsp.window.resize(), { mouse **\= true** }) 

hl.bind("SUPER \+ ALT\_L", hl.dsp.window.resize(), { mouse **\= true** }) 
...
See the pass and send\_shortcut dispatchers for keybinds. 
...
hl.bind("mouse:276", hl.dsp.pass({window **\=** "class:^(TeamSpeak 3)$"})) **\#** Pass MOUSE5 to TeamSpeak3. You may also add shortcuts, where other keys are passed to the window. 

hl.bind("SUPER \+ F10", hl.dsp.send\_shortcut({ mods **\=** "SUPER", key **\=** "F4", window **\=** "class:^(TeamSpeak 3)$
...
Some applications may already support the GlobalShortcuts portal in xdg-desktop-portal. If that’s the case, it’s recommended to use the following method instead of pass : 
...
hl.define\_submap("resize", **function**() 

\-- Set repeating binds for resizing the active window. 

hl.bind("right", hl.dsp.window.resize({ x **\=** 10, y **\=** 0, relative **\= true**}), { repeating **\= true** }) hl.bind("left", hl.dsp.window.resize({ x **\= \-**10, y **\=** 0, relative **\= true**}), { repeating **\= true** }) hl.bind("up", hl.dsp.window.resize({ x **\=** 0, y **\=** 10, relative **\= true**}), { repeating **\= true** }) hl.bind("down", hl.dsp.window.resize({ x **\=** 0, y **\= \-**10, relative **\= true**}), { repeating **\= true** }) 
...
You can set a keybind that will be active no matter the current submap with the submap universal bind flag. hl.bind(mainMod **..** " \+ K", hl.dsp.exec\_cmd("kitty"), { submap\_universal **\= true** }) 

Nesting 

Submaps can be nested, see the following example: 

hl.bind(mainMod **..** " \+ M", hl.dsp.submap("main\_submap")) 

hl.define\_submap(main\_submap, **function**() 

\-- ... 

\-- nested\_one 

hl.bind("1", hl.dsp.submap("nested\_one")) 

hl.define\_submap("nested\_one", **function**() 

\-- ... 

hl.bind("SHIFT \+ escape", hl.dsp.submap("reset")) 

hl.bind("escape", hl.dsp.submap("main\_submap")) 

\-- nested\_two 

hl.bind("2", hl.dsp.submap("nested\_two")) 

hl.define\_submap("nested\_two", **function**() 
...
hl.define\_submap("submapA", "submapB", **function**() 

hl.bind("a", hl.dsp.exec\_cmd("someCoolThing.sh")) 
...
hl.bind("a", hl.dsp.exec\_cmd("someOtherCoolThing.sh")) 
...
kb\_options **\=** "grp:alt\_shift\_toggle" 
...
For example: us,ua \-\> config binds would be e.g. "SUPER \+ A" , while on ua,us \-\> "SUPER \+ Cyrillic\_ef 

You can change this behavior globally or per-device by setting resolve\_binds\_by\_sym \= 1 . In that case, binds will activate when the symbol typed matches the symbol specified in the bind. 
...
To find the valid layouts and kb\_options , you can check out the /usr/share/X11/xkb/rules/base.lst . For example: 

To get the layout name of a language: 

grep \-i 'persian' /usr/share/X11/xkb/rules/base.lst 

To get the list of keyboard shortcuts you can put in the kb\_options to toggle keyboard layouts: grep 'grp:.\*toggle' /usr/share/X11/xkb/rules/base.lst

Misc 

Workspace bindings on non-QWERTY layouts 

Keys used for keybinds need to be accessible without any modifiers in your layout. For instance, the French AZERTY layout uses SHIFT \+ unmodified key to write 0-9 numbers. As such, the   
workspace keybinds for this layout need to use the names of the unmodified keys , and will not work when using the 0-9 numbers. 

Note 

To get the correct name for an unmodified\_key , refer to the section on uncommon syms 
...
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec\_cmd("wpctl set-volume @DEFAULT\_AUDIO\_SINK@ 5%+"), { repeating hl.bind("XF86AudioLowerVolume", hl.dsp.exec\_cmd("wpctl set-volume @DEFAULT\_AUDIO\_SINK@ 5%-"), { repeating hl.bind("XF86AudioMute", hl.dsp.exec\_cmd("wpctl set-mute @DEFAULT\_AUDIO\_SINK@ toggle"), { locked **\=** 

\-- Requires playerctl 

hl.bind("XF86AudioPlay", hl.dsp.exec\_cmd("playerctl play-pause"), { locked **\= true** }) hl.bind("XF86AudioPrev", hl.dsp.exec\_cmd("playerctl previous"), { locked **\= true** }) hl.bind("XF86AudioNext", hl.dsp.exec\_cmd("playerctl next"), { locked **\= true** })
...
hl.define\_submap("clean", **function**() 
...
kb\_options **\=** "caps:swapescape" 

} 

})

You can also find additional kb\_options unrelated to Caps Lock in /usr/share/X11/xkb/rules/base.lst . Last updated on June 2, 2026 