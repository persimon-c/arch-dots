Configuring Advanced and Cool Permissions 
...
If you have hyprland-guiutils installed, you can make use of Hyprland’s built-in permission system. For now, it only has a few permissions, but it might include more in the future. 
...
Before setting up permissions, make sure you enable them by setting hl.config({ ecosystem \= { enforce\_permissions \= true } }) , as it’s disabled by default. 
...
Will allow any app whose path starts with /usr/bin/appsuite- to capture your screen without asking. Permission modes 
...
hl.permission({ binary **\=** "/usr/(lib|libexec|lib64)/xdg-desktop-portal-hyprland", type **\=** "screencopy", mod if you are not allowing screencopy for all apps. 
...
hl.permission({ binary **\=** "/nix/store/\[a-z0-9\]{32}-grim-\[0-9.\]\*/bin/grim", type **\=** "screencopy", mode **\=** "al hl.permission({ binary **\=** "/nix/store/\[a-z0-9\]{32}-xdg-desktop-portal-hyprland-\[0-9.\]\*/libexec/.xdg-deskto
...
hl.permission({ binary **\=** "${lib.getExe pkgs.grim}", type **\=** "screencopy", mode **\=** "allow" }) hl.permission({ binary **\=** "${lib.escapeRegex (lib.getExe config.programs.hyprlock.package)}", type **\=** "scre hl.permission({ binary **\=** "${pkgs.xdg-desktop-portal-hyprland}/libexec/.xdg-desktop-portal-hyprland-wrappe 
