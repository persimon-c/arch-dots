Configuring Advanced and Cool Animations 
...
enabled use true to enable, false to disable. Note: if it’s false , you can omit further args. speed is the amount of ds (1ds \= 100ms) the animation will take. For example speed \= 1 \= 100ms bezier / spring is the curve name, see curves. 

style (optional) is the animation style. See Animation tree 

Examples 

hl.animation({ leaf **\=** "workspaces", enabled **\= true**, speed **\=** 8, bezier **\=** "my\_epic\_bezier" }) hl.animation({ leaf **\=** "window", enabled **\= true**, speed **\=** 10, spring **\=** "my\_epic\_spring", style **\=** "slide"}) hl.animation({ leaf **\=** "fade", enabled **\=** 0 }) 
...
↳ borderangle \- for animating the border's gradient angle \- styles: once (default), loop ↳ workspaces \- styles: slide, slidevert, fade, slidefade, slidefadevert  
...
A cubic Bézier curve is a simple spline defined by 4 points, two of which (the middle ones) are configurable. hl.curve( NAME, { type **\=** "bezier", points **\=** { {X0, Y0}, {X1, Y1} } }) 

A good website to design your own Bézier can be cssportal.com. 

If you want to instead choose from a list of pre-made Béziers, you can check out easings.net. Spring 
...
The more “stiffness”, the more speed, and the more “dampening”, the less bounce. Examples 

hl.curve( "overshoot", { type **\=** "bezier", points **\=** { {0.5, 0.9}, {0.1, 1.1} } } ) hl.curve( "rubber", { type **\=** "spring", mass **\=** 1, stiffness **\=** 70, dampening **\=** 10 } ) 
...
hl.animation({ leaf **\=** "windows", enabled **\= true**, speed **\=** 8, curve **\=** "default", style **\=** "slide left" })Last updated on June 2, 2026 