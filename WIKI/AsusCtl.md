Updated AsusCtl.qml scope:

Power profile — query + asusctl profile set / asusctl profile next
Aura/RGB mode — query + asusctl led-mode -n / asusctl led-mode -p (next/prev only, no color picking since it's single-zone preset-based)
GPU mode — query + supergfxctl -m <mode> + reboot warning
Panel overdrive — query + asusctl armoury set panel_overdrive <1/0>
...
Installation
Install the asusctlAUR package or alternatively from a custom repository.
...
Note
This is only necessary if your laptop supports custom effects, predefined effects (such as Pulse, Static and Breathe) should work with no extra steps.
...
Note
A charging limit of 75 is suitable for a laptop that is unplugged often. 60 is a good value for one that is left plugged in most of the time
RGB/Keyboard backlight
...
$ asusctl armoury set panel_overdrive <1/0>
Power profiles
...
Note
Power profile switching requires the power-profiles-daemon package to be running.
...
"brightness": <FLOAT>: global brightness control, where ` is 0.0-1.0
